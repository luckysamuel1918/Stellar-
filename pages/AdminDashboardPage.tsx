    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg">
                <h2 className="text-xl font-bold text-westcoast-dark mb-2">Manage Balance</h2>
                <p className="text-westcoast-text-light mb-6">For <span className="font-semibold">{user.fullName}</span></p>
                <form onSubmit={handleSubmit} className="space-y-4">
                     <div>
                        <label className="block text-sm font-medium text-westcoast-text-light">Transaction Type</label>
                     ...
                     {type === 'credit' && (
                        <div>
                            <label className="block text-sm font-medium text-westcoast-text-light">Sender's Name</label>
                            <input type="text" value={senderName} onChange={e => setSenderName(e.target.value)} placeholder="e.g., John Doe" className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg" />
                        </div>
                    )}
                    <div>
                        <label className="block text-sm font-medium text-westcoast-text-light">Amount ({user.currencyCode})</label>
                     ...
                     <div>
                        <label className="block text-sm font-medium text-westcoast-text-light">Purpose of Transfer / Description</label>
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-westcoast-text-light">Transaction Date</label>
                            <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg" required />
                        </div>
                         <div>
                            <label className="block text-sm font-medium text-westcoast-text-light">Transaction Time</label>
                            <input type="time" value={time} onChange={e => setTime(e.target.value)} className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg" required />
                        </div>
                    </div>
...
const EditUserModal = ({ user, onClose, onUpdate }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg">
                <h2 className="text-xl font-bold text-westcoast-dark mb-6">Edit User Profile</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
...
const InputField = ({ label, ...props }) => (
    <div>
        <label className="block text-sm font-medium text-westcoast-text-light">{label}</label>
        <input {...props} className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg" />
    </div>
);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md text-center">
                ...
                <h2 className="text-xl font-bold text-westcoast-dark">Confirm Deletion</h2>
                <p className="text-westcoast-text-light my-4">Are you sure you want to permanently delete <span className="font-bold">{user.fullName}</span>? This will also remove all their transactions and chat history. This action cannot be undone.</p>
                {error && <p className="text-red-500 text-sm mb-4">{error}</p>}
                <div className="flex justify-center space-x-4">
                    <button onClick={onClose} className="px-6 py-2 bg-gray-200 text-gray-800 font-semibold rounded-lg">Cancel</button>
    ...
    return (
       <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl max-h-[90vh] flex flex-col">
                <h2 className="text-xl font-bold text-westcoast-dark mb-4">Transactions for {user.fullName}</h2>
                <div className="overflow-y-auto flex-grow">
                    {loading ? <p>Loading...</p> : (
                        <ul className="space-y-3">
                            {transactions.map(tx => (
                                <li key={tx.id} className="p-3 bg-gray-50 rounded-lg">
                                    {editingTx?.id === tx.id ? (
                                        <div className="space-y-2">
                                            <div className="grid grid-cols-2 gap-2">
                                                <input type="date" value={date} onChange={e => setDate(e.target.value)} className="px-2 py-1 border rounded" />
                                                <input type="time" value={time} onChange={e => setTime(e.target.value)} className="px-2 py-1 border rounded" />
                                            </div>
                                            <div className="flex justify-end gap-2">
                                                <button onClick={() => setEditingTx(null)} className="text-xs px-2 py-1 bg-gray-200 rounded">Cancel</button>
                ...
                <button onClick={onClose} className="mt-4 w-full py-2 bg-gray-200 rounded-lg font-semibold">Close</button>
            </div>
        </div>
    );
};
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg h-[80vh] flex flex-col">
                <header className="p-4 border-b flex justify-between items-center">
                    <div>
                        <h2 className="text-lg font-bold text-westcoast-dark">Chat with {user.fullName}</h2>
                        <p className="text-xs text-gray-500">User ID: {user.uid}</p>
                    </div>
                    <div>
                        <button onClick={handleWipe} className="mr-4 p-2 text-red-600 hover:bg-red-100 rounded-full"><Trash2 size={18} /></button>
                        <button onClick={onClose} className="p-2 text-gray-600 hover:bg-gray-100 rounded-full"><X size={20} /></button>
                    </div>
                </header>
                    ...
                    <div className="space-y-4">
                        {messages.map(msg => (
                             <div key={msg.id} className={`flex ${msg.senderId === 'admin' ? 'justify-end' : 'justify-start'}`}>
                                 <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-xl ${msg.senderId === 'admin' ? 'bg-westcoast-blue text-white' : 'bg-gray-200 text-black'}`}>
                                    <p className="text-sm">{msg.text}</p>
                                    <p className={`text-xs mt-1 ${msg.senderId === 'admin' ? 'text-blue-200' : 'text-gray-500'}`}>{msg.timestamp?.toDate().toLocaleTimeString()}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <footer className="p-4 border-t">
                    <form onSubmit={handleSend} className="flex gap-2">
                        <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type a message..." className="flex-grow px-4 py-2 border rounded-full focus:outline-none focus:ring-2 focus:ring-westcoast-blue" />
        ...
        <div className="bg-westcoast-bg min-h-[calc(100vh-150px)] p-4 sm:p-6 md:p-8">
            ...
            <div className="max-w-7xl mx-auto">
                <header className="mb-8">
                    <h1 className="text-3xl sm:text-4xl font-bold text-westcoast-dark">Admin Dashboard</h1>
                    <p className="text-westcoast-text-light">Welcome, {userData?.fullName}!</p>
                </header>

                <div className="bg-white p-6 rounded-xl shadow-md">
                    <h2 className="text-xl font-bold text-westcoast-dark mb-4">User Management</h2>
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tbody>
                                {users.map(user => (
                                    <tr key={user.uid} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                                        <td className="py-3 px-4">
                                            <p className="font-semibold text-westcoast-text-dark">{user.fullName}</p>
                                            <p className="text-sm text-westcoast-text-light">{user.email}</p>
                                        </td>
                                        <td className="py-3 px-4 text-sm text-westcoast-text-light font-mono">{user.accountNumber}</td>
                                        <td className="py-3 px-4 font-mono text-right font-semibold">{user.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {user.currencyCode}</td>
                                        <td className="py-3 px-4 text-center">
                                            <div className="flex items-center justify-center space-x-2">
                                                 <button onClick={() => openModal('balance', user)} title="Manage Balance" className="text-green-600 hover:text-green-800 p-2 rounded-full hover:bg-green-100"><DollarSign size={18} /></button>
                                                 <button onClick={() => openModal('edit', user)} title="Edit Profile" className="text-blue-600 hover:text-blue-800 p-2 rounded-full hover:bg-blue-100"><Edit size={18} /></button>
                                                 <button onClick={() => openModal('transactions', user)} title="Manage Transactions" className="text-purple-600 hover:text-purple-800 p-2 rounded-full hover:bg-purple-100"><Clock size={18} /></button>
                                                 <button onClick={() => openModal('chat', user)} title="Chat with User" className="text-cyan-600 hover:text-cyan-800 p-2 rounded-full hover:bg-cyan-100"><MessageSquare size={18} /></button>
                                                 <button onClick={() => openModal('delete', user)} title="Delete User" className="text-red-600 hover:text-red-800 p-2 rounded-full hover:bg-red-100"><Trash2 size={18} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
