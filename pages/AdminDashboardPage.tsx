    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-md">
                <h2 className="text-2xl font-bold text-westcoast-dark mb-2">Manage Balance</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                     ...
                     <div>
                        <label className="block text-sm font-medium text-westcoast-text-light">Description</label>
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-xl shadow-md flex items-center">
                        <div className="bg-westcoast-blue/10 p-4 rounded-full mr-4"><Users className="text-westcoast-blue"/></div>
                        <div>
                            <p className="text-sm font-medium text-westcoast-text-light">Total Users</p>
                            <p className="text-2xl sm:text-3xl font-bold text-westcoast-text-dark">{users.length}</p>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                {users.map(user => (
                                    <tr key={user.uid} className="border-b border-gray-100 last:border-0">
                                        <td className="py-4 px-4 font-semibold text-westcoast-text-dark">{user.fullName}</td>
                                        <td className="py-4 px-4 text-sm text-westcoast-text-light">{user.email}</td>
                                        <td className="py-4 px-4 text-sm text-westcoast-text-light font-mono">{user.accountNumber}</td>
                                        ...
                                        <td className="py-4 px-4 text-center">
                                            <button onClick={() => setSelectedUser(user)} className="text-westcoast-blue hover:text-westcoast-accent p-1">
                                                <Edit size={18} />
                                            </button>
                                        </td>
                                    </tr>
